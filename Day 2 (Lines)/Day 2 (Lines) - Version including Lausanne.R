...
Lausanne <- st_read(here("Data", "Lausanne RM-Amenagement-cyclable", "velo_amenagement.shp"))
...
LausannePlot <- ggplot() +
  geom_sf(data = Lausanne, aes(geometry = geometry), size = linesize, color = color4) +
  theme_void() +
  theme(plot.margin = margin(topmargin, 1, 0, 1, unit = "cm"))
...
GridPlot <- plot_grid(BarcelonaPlot, BerlinPlot, ChicagoPlot, LausannePlot, LondonPlot,
...
ggsave(here("30day map challenge - day 2 - lines - incl Lausanne.png"), width = 29.7, height = 21)
